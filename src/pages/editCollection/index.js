...
const EditCollection = () => {
  ...
  const data = useSelector((state) => state.nftReducer.collectionDataById);

  useEffect(() => {
    if (data?.data) {
      setfeatureImage(data?.data[0]?.featuredImage);
      setbannerImage(data?.data[0]?.bannerImage);
      setlogoImage(data?.data[0]?.logoImage);
      formik.setFieldValue("name", data?.data[0]?.name);
      formik.setFieldValue("description", data?.data[0]?.description);
      formik.setFieldValue("url", data?.data[0]?.collectionUrl);
      setselectedBlockChain({
        name: data?.data[0]?.blockChain,
        value: 1,
      });
      setpaymentToken({
        id: 1,
        token: data?.data[0]?.paymentToken,
        name: "Ethereum",
        image: "/static/images/ETH.svg",
      });
      setexplictAndSensitiveContent(data?.data[0]?.explictAndSensitiveContent);
  ...
  useEffect(() => {
    ...
    dispatch(getCollection_ById.request(temporaryData?.collectionId?._id));
  }, [dispatch]);
  ...
  const formik = useFormik({
    ...
    onSubmit: async () => {
      ...
      let input = {
        body,
        id: temporaryData?.collectionId?._id,
      };
      console.log(body, "jijijij");
      dispatch(updateCollection.request(input));
      setTimeout(() => {
      ...
      <Box className="py-60">
        <Box className="conta_iner">
          ...
          <Box>
            ...
            <form onSubmit={formik.handleSubmit} action="" className="mt-20">
              <Box>
                {/* ----------- LOGO IMAGE ---------- */}
                <Box className="logo_Image mb-30">
                ...
                <Box className="banner_image  ">
                  ...
                  <Box className="imgUpld_wpr bnr_wpr mb-30">
                    <label
                ...
                <Box className="input-group mb-30">
