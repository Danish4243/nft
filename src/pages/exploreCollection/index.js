...
const ExploreCollection = () => {
  ...
  const categoriesData = useSelector((state) => state.nftReducer.categoryData);
  ...
  const categorySelect = (item) => {
    ...
    dispatch(getCollection.request(item?.name));
  };
  ...
  return (
    <Box>
      ...
      <Box className="conta_iner">
        ...
        <Box className="explor_item">
          <Box
            className="inr_xplore"
            onClick={() => {
              setselectedFilter("All"), dispatch(getCollection.request());
            }}
            style={{
          ...
          {categoriesData?.data?.map((item, index) => {
            return (
              <Box
                ...
                style={{
                  backgroundColor: selectedFilter.includes(item.name)
                    ? "yellow"
                    : "#F2F2F2",
                }}
              >
